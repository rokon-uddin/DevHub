...
let mockUsers = try! JSONDecoder().decode([User].self, from: usersJson.data(using: .utf8)!)
let mockUsersNext = try! JSONDecoder().decode([User].self, from: usersJsonNext.data(using: .utf8)!)
